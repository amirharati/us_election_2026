...
## Available forecasts and conditional settlement estimates

Every selected market keeps all six model rows. Local state-winner probabilities are shown when the modeled side can be matched to the contract, including runoff and ranked-choice races. A `*` on the assessment marks a **conditional settlement proxy**: the calculation uses the model's margin distribution as an approximation to the eventual winner and does not separately simulate runoff turnout, ranked-choice transfers, or other-candidate outcomes. Color follows the assessment: GO* is green, NEG* is red, and WEAK*/UNC* are amber. The star independently flags the settlement assumption. The same depth, fees, EV and stress formulas apply, conditional on that approximation. Notes beneath the table identify the affected model codes and assumptions.

Single independent contenders can map to an independent or exact-name contract; they never become Democrats. When DEM and IND contenders compete on the same modeled side, the scalar strongest-opponent forecast cannot allocate probability between them. Those candidate/party-specific contracts remain unavailable, while the Republican-side complement can appear as a labeled approximation. A party-winner forecast without a reviewed roster is shown only as an explicit unreviewed-roster D/R approximation; unknown named candidates stay unavailable.

This does not relax margin-contract settlement checks. First-round events, incompatible margin definitions and missing external distributions remain unpriced. Unavailable local rows now show their precise reasons below the table alongside publisher notes. Forecasts with stale evidence or failed refreshes retain their existing eligibility rules.
