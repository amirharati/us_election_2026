...
Every selected market keeps all six model rows. Local state-winner probabilities are shown when the modeled side can be matched to the contract, including runoff and ranked-choice races. A `*` on the assessment marks a **conditional settlement proxy**: the calculation uses the model's margin distribution as an approximation to the eventual winner and does not separately simulate runoff turnout, ranked-choice transfers, or other-candidate outcomes. These rows stay amber even if their numerical edge survives stress. The same depth, fees, EV and stress formulas apply, conditional on that approximation. Notes beneath the table identify the affected model codes and assumptions.
